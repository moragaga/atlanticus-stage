...
class Observability:
    ...
    def __post_init__(self) -> None:
        # La composición explícita se valida una vez antes de convertirse en el estado del proceso.
...
def emit_event(event: ObservabilityEvent) -> bool:
    ...
    try:
        ...
        # La observabilidad nunca puede interrumpir el flujo de negocio por un fallo propio.
        return False
    return True
    ...
    *,
    ...
    try:
        ...
        if handle is not None:
            try:
                handle.end(
                    # El bridge recibe sólo el tipo y un mensaje generado internamente.
                    SpanError(
...
def _validate_trace_bridge(trace_bridge: Any) -> None:
    # TraceBridge es un protocolo; se valida por los dos métodos requeridos sin acoplar un proveedor.
