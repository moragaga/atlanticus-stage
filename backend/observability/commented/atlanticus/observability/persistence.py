...
def resolve_observability_root(
    ...
    *,
    ...
    # La raíz pertenece a la aplicación y conserva el contrato VOLUMEN/APPLICATION/logs.
...
class AtomicJsonlWriter:
    ...
    def append(self, path: Path, payload: Mapping[str, Any], *, durable: bool = False) -> None:
        # Un lock de hilo protege cada registro; la coordinación entre procesos pertenece al runtime.
...
class AtomicJsonWriter:
    ...
    def replace(self, path: Path, payload: Mapping[str, Any]) -> None:
        # El temporal vive en el mismo directorio para que os.replace mantenga atomicidad local.
        if not isinstance(path, Path):
            raise TypeError('path must be a Path')
        if not isinstance(payload, Mapping):
            raise TypeError('payload must be a mapping')
...
class DailyTraceSink(EventSink):
    ...
    def __init__(
        ...
        *,
        ...
        # Las dependencias del sink se validan antes de crear directorios o abrir archivos.
        if not isinstance(volume_path, str | Path):
            raise TypeError('volume_path must be a string or Path')
    ...
    def _purge_previous_iso_weeks(service_root: Path, reference_day: date) -> None:
        # Sólo se eliminan hijos con el patrón diario exacto y pertenecientes a semanas anteriores.
