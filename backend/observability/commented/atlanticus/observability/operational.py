...
class OperationalEventProjection(EventProjection):
    ...
    def includes(self, event: ObservabilityEvent) -> bool:
        # La selección se basa en semántica operacional, no en el destino físico del evento.
    ...
    def project(
        ...
        if not self.includes(event):
            return None
        # Los campos mínimos son obligatorios porque identifican cuándo ocurrió el hecho y su nivel.
    ...
    def _merge_values(cls, destination: dict[str, Any], values: Any) -> None:
        # Métricas y atributos seguros se aplanan sin permitir que reemplacen campos reservados.
