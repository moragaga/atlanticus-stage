...
# Los helpers concentran las reglas comunes para que cada contrato falle al construirse y no al
# momento de emitir un evento.
...
def _freeze_value(value: Any) -> Any:
    # Las colecciones se convierten recursivamente a equivalentes inmutables. Así el evento conserva
    # una fotografía estable aunque el consumidor modifique más tarde sus objetos originales.
...
def _exception_traceback(error: BaseException) -> str | None:
    # El diagnóstico conserva archivo, línea y función, pero omite deliberadamente los mensajes de
    # las excepciones porque un SDK puede incluir credenciales o URLs firmadas en ellos.
    sections: list[str] = []
...
class ExecutionContext:
    ...
    def __post_init__(self) -> None:
        # Los identificadores opcionales deben ser coherentes antes de propagarse entre scopes.
...
class ErrorInfo:
    ...
    def from_exception(cls, error: BaseException) -> ErrorInfo:
        ...
        # Este es el único constructor automático desde excepciones. Los mappers explícitos pueden
        # crear ErrorInfo directamente cuando poseen un mensaje controlado y seguro.
...
class ObservabilitySettings:
    ...
    def __post_init__(self) -> None:
        ...
        # resueltos para impedir estados parcialmente válidos.
...
class ObservabilityEvent:
    ...
    def __post_init__(self) -> None:
        # La validación completa ocurre una sola vez. Después el evento puede compartirse entre
        # múltiples sinks sin que cada destino tenga que defenderse de contratos inválidos.
...
class ResultSummary:
    ...
    def __post_init__(self) -> None:
        # Se reutilizan exactamente las reglas de métricas y atributos del evento para evitar dos
        # interpretaciones distintas del mismo payload.
