# Espejo pedagógico: concentra invariantes tabulares puras y deja al store como orquestador.
...
# La alineación proyecta columnas faltantes como nulos y rechaza cambios de tipo incompatibles.
def _align_table(
    *,
    table: pa.Table,
    schema: pa.Schema,
...
def _normalize_projection_schema(value: pa.Schema | None) -> pa.Schema | None:
    # El schema tipado habilita tolerancia sólo cuando el caller declara tipos explícitos.
    if value is None:
        return None
    ...
    # Un tipo null no aporta la información necesaria para sintetizar una columna útil.
