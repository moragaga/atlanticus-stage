# Espejo pedagógico: concentra cómo se localiza y valida la publicación física vigente.
...
def _resolve_publication(
    *,
    ...
    # La definición decide el layout; el resolver no descubre rutas por su cuenta.
    materialization = definition.get_materialization(target.materialization)
    definition.validate_target(target)
    # Un artefacto único no usa manifest: data.parquet es la publicación confirmada.
    ...
    # Un file-set se considera confirmado exclusivamente por current.json.
    ...
    # Cada parte seleccionada se inspecciona contra tamaño, hash, filas y schema del manifest.
...
def _inspect_artifact(
    *,
    ...
    # Primero verificamos la identidad física antes de abrir metadata Parquet.
    try:
    ...
    # La metadata confirma que el archivo es Parquet legible y aporta schema/row count.
    try:
...
def _read_manifest(
    *,
    ...
    # current.json es el único puntero mutable hacia el file-set vigente.
...
def _select_manifest_parts(
    *,
    ...
    # El filtro por dimensión de parte evita inspeccionar archivos que no pueden participar.
...
def _validate_preserved_parts(
    *,
    ...
    # Antes de confirmar un manifest nuevo, las partes preservadas deben seguir siendo válidas.
    for part in parts:
        artifact = _inspect_artifact(
            path=target_path / part.path,
            missing_is_publication=False,
...
def _validate_physical_schema(
    *,
    ...
    # El schema lógico puede agregar columnas, pero una columna existente no puede cambiar de tipo.
