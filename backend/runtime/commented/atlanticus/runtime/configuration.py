# La configuración conserva solo identidad, ambiente y volumen; no transporta secretos.
...
# Conserva el mismo vocabulario booleano que ResolvedConfiguration.get_bool().
...
class RuntimeConfiguration:
    ...
    application: str
    volume_path: Path
    # Los jobs mantienen persistencia por defecto; cada runtime puede apagarla vía ENV.
...
def _optional_bool(values: Mapping[str, str], name: str, *, default: bool) -> bool:
    # El parser es estricto: no recorta espacios y falla si el valor no pertenece al contrato.
    raw_value = values.get(name)
