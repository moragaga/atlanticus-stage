# Mantiene revisión y watermark por modo antes de completar mensajes.
...
class NotPiiProducerState:
    def __init__(
        ...
        *,
        ...
        if not isinstance(producer_key, str) or not producer_key.strip():
            raise TypeError('producer_key must be a non-empty string')
        self._store = store
    ...
    def advance(
        ...
        if state_changed:
            self._store.replace(
                self._state_key,
                _manifest_value(manifest, producer_key=self._producer_key),
            )
