...
def test_commented_tree_is_behaviorally_equivalent() -> None:
    ...
    source_root = project_root / 'src' / 'atlanticus' / 'data_producers' / 'notpii'
    commented_root = project_root / 'commented' / 'atlanticus' / 'data_producers' / 'notpii'
    ...
    for relative in source_files:
        source_ast = ast.dump(
            ast.parse((source_root / relative).read_text()),
            include_attributes=False,
        )
        commented_ast = ast.dump(
            ast.parse((commented_root / relative).read_text()),
            include_attributes=False,
        )
