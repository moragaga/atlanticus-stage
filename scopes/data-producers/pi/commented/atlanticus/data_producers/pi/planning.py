# Espejo comentado del productor PI reutilizable: planning.py
...
from atlanticus.data_producers.pi.errors import PiDataProducerPlannerError
from atlanticus.data_producers.pi.models import PiAcquisitionWindow
...
class PiSlotPlanner:
    ...
    def __post_init__(self) -> None:
        for field_name in (
            ...
            if not isinstance(value, int) or isinstance(value, bool) or value <= 0:
                raise PiDataProducerPlannerError(f'{field_name} must be a positive integer')
        for field_name in (
            'max_recovery_lookback_seconds',
            'max_recovery_window_seconds',
        ):
            ...
            if value < self.interpolation_seconds:
                raise PiDataProducerPlannerError(
    ...
    def next_slot(self, value: datetime) -> datetime:
        ...
        if self.floor_slot(normalized) != normalized:
            raise PiDataProducerPlannerError('value must be aligned to interpolation_seconds')
...
def _require_utc(value: datetime, *, field_name: str) -> datetime:
    if not isinstance(value, datetime) or value.tzinfo is None:
        raise PiDataProducerPlannerError(f'{field_name} must be a timezone-aware datetime')
    if value.utcoffset() != timedelta(0):
        raise PiDataProducerPlannerError(f'{field_name} must use UTC')
...
def _require_utc_second(value: datetime, *, field_name: str) -> datetime:
    ...
    if normalized.microsecond != 0:
        raise PiDataProducerPlannerError(f'{field_name} must not contain microseconds')
    return normalized
