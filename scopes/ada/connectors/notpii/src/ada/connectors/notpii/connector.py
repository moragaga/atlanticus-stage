...
from ada.connectors.notpii._wide import build_wide
from ada.connectors.notpii.errors import NotPiiConfigurationError, NotPiiSourceError
from ada.connectors.notpii.models import (
    NotPiiBatch,
    NotPiiBlobMessage,
    optional_datetime,
    optional_text,
...
class NotPiiConnector:
    def __init__(self, *, storage_reader: StorageSasReader, raw_batch_size: int = 100_000) -> None:
        if not isinstance(storage_reader, StorageSasReader):
            raise NotPiiConfigurationError('storage_reader must be a StorageSasReader')
        if (
            not isinstance(raw_batch_size, int)
            or isinstance(raw_batch_size, bool)
            or raw_batch_size <= 0
        ):
            raise NotPiiConfigurationError('raw_batch_size must be greater than zero')
    ...
    def read(
        ...
        *,
        ...
        if not isinstance(message, ServiceBusMessage):
            raise NotPiiConfigurationError('message must be a ServiceBusMessage')
        if not isinstance(catalog, PiCatalog):
            raise NotPiiConfigurationError('catalog must be a PiCatalog')
        if not isinstance(catalog.source, NotPiiSource):
            raise NotPiiConfigurationError('catalog source must be NotPiiSource')
        ...
        if not selected:
            raise NotPiiConfigurationError('catalog does not contain the requested extraction mode')
