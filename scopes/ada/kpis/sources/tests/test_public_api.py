...
def test_public_api() -> None:
    ...
    assert sources.KpiPartitionBinding is not None
    assert sources.KpiSourceRegistry is not None
    assert sources.LoadedKpiSourceView is not None
    assert sources.PandasRuntimeFrameContext is not None
    assert sources.KpiOperationalWindowResolver is not None
