...
# El bundle solo es válido para el intérprete exacto acordado por Atlanticus.
...
# Los mismos residuos se excluyen al copiar procesos y al construir dependencias.
...
# Error de dominio para impedir que detalles de subprocess o TOML escapen al usuario.
class ProcessBundleError(RuntimeError):
    pass


# Representa únicamente la metadata necesaria para resolver el cierre interno.
...
# Recorre el monorepo y crea un índice único por nombre canónico de distribución.
...
# Valida el contrato que luego consumirá el Dockerfile genérico.
...
# Recorre dependencias internas en orden topológico y exige versiones exactas.
...
# Genera primero en una carpeta temporal para no dejar bundles incompletos.
def build_process_bundle(
    *,
    ...
    try:
        # Conserva código, pruebas, espejo comentado y archivos operacionales del proceso.
        _copy_process_project(source=process_root, target=temporary_project)
        # Cada dependencia Atlanticus o ADA se materializa como wheel propio del proceso.
        wheel_sources = _build_internal_wheels(
            repository_root=repository_root,
            dependencies=dependencies,
            wheel_directory=temporary_project / 'wheels',
        )
        # La copia exportada apunta a wheels relativos y deja de depender del monorepo.
        ...
        # El lock se recalcula sin caché para evitar reutilizar wheels antiguos con igual versión.
        _run(
            (
                'uv',
                'lock',
...
# Copia el proyecto completo, excluyendo únicamente artefactos regenerables.
...
# Toda construcción trabaja sobre una copia descartable y nunca modifica el monorepo.
...
# Construye cada wheel en una salida separada para detectar resultados ambiguos.
def _build_internal_wheels(
    *,
    ...
    try:
        for dependency in dependencies:
            # Setuptools puede crear egg-info, por eso nunca construye sobre el origen real.
...
# Añade fuentes uv por archivo exacto sin modificar el pyproject original.
# Completa el pyproject exportado con las fuentes locales de todo el cierre interno.
...
# Comprueba que el entry point declarado fue instalado realmente por uv.
...
# Exporta uno, varios o todos los procesos que declaran contrato de contenedor.
def main() -> None:
