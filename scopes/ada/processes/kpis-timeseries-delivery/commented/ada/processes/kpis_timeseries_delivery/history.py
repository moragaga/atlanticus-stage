...
# Lee únicamente la ventana y columnas necesarias del Historian existente.
        ...
        for day in _days(timestamps[0].date(), timestamps[-1].date()):
            ...
            try:
                result = self._runtime.scan_table(
                    ...
                    targets=(target,),
                    # Series sólo proyecta el valor histórico; no interpreta estados KPI.
...
def _point(record: dict[str, object]) -> KpiTimeseriesPoint:
    ...
    # Un valor ausente se mantiene como null y no se intenta inferir su causa.
