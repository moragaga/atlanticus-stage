...
# R3.1 mantiene la sesión como contrato puro: congela revisiones, evaluadores, parámetros y el plan de datos,
# pero deliberadamente no incorpora lectura física ni un reloj de iteración. Ambos pertenecen a R3.2.
...
# El contrato registrado representa código permitido por family/evaluator key; configuración nunca entrega callables arbitrarios.
...
# El registry queda residente en la sesión y resuelve únicamente por las keys ya presentes en PlannedAlarm.
@dataclass(frozen=True, slots=True)
...
# Cada entry une una alarma ya planificada con el contrato registrado resuelto y parámetros estáticos copiados.
@dataclass(frozen=True, slots=True)
...
# La sesión es la frontera congelada de una execution window futura. No contiene `now` porque ese valor es por iteración.
@dataclass(frozen=True, slots=True)
class AlarmExecutionSession:
    ...
    def __post_init__(self) -> None:
        ...
        # Una sesión nunca mezcla revisiones. El reemplazo por una nueva revisión deberá construir otra sesión completa.
...
# El builder resuelve contratos registrados y delega toda consolidación source/partition/column al planner compartido.
...
# Copiar a MappingProxyType evita que una mutación externa cambie parámetros dentro de una sesión ya validada.
