...
# Error de frontera cuando la composición recibe dependencias o estados incompatibles.
class AlarmRuntimeJobCompositionError(RuntimeError):
    pass


# Resultado de adopción que la orquestación expone sin persistir semántica adicional.
...
# Resume una iteración para pruebas, logs y observabilidad posterior.
...
# Compone los contratos ya cerrados en una sola unidad ejecutable por Job Runtime.
class AlarmRuntimeJobComposition:
    ...
    # Recovery sólo alinea durable y materializado; no resuelve ni adopta configuración.
    ...
    # El primer epoch sólo puede existir cuando no hay WAL ni consumer state previos.
    def _bootstrap(
        self,
    ...
    # La transición usa effective y target ya resueltos, sin releer el cache.
    ...
    # Datos, consumer y ciclo comparten la misma sesión efectiva congelada.
    ...
    # El cache se promueve dentro de un único fence y sólo después de la autoridad durable requerida.
    def _promote_cache(
        self,
        context: JobRuntimeContext,
        resolution: RuntimeRevisionResolution,
    ...
    # Impide procesar trabajo nuevo antes de completar recovery.
    ...
    # Distingue bootstrap real de pérdida del cache con historia durable existente.
