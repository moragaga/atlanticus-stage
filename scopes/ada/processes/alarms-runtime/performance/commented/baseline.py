...
# F-007 agrega un loader físico inyectable sin reemplazar el loader sintético usado por el resto de la campaña.
...
# Mantenemos 1.000 contratos evaluadores fijos, uno por señal física; las alarmas adicionales reutilizan esos contratos sin ensanchar el dataset.
...
# F-010 combina Latest físico, histórico DAILY, control-plane y una adopción compatible en una sola ejecución.
...
class BaselineScenario:
    ...
    # E-012 activa una frontera cooperativa de stop/drain sobre el workload mixto.
    # El valor es tiempo lógico del escenario; no representa un timeout de proceso.
    drain_under_workload_at_seconds: int = 0
    # El soak reutiliza el recorder existente: sólo declaramos cuánto warmup excluir
    # y el tamaño de las ventanas estables que luego se compararán.
    ...
    def __post_init__(self) -> None:
        ...
        if self.deactivation_decision_at_seconds > 0:
            ...
            # E-012 reutiliza intencionalmente la geometría D-009 para que el prefijo
            # durable de +300 s sea determinista y auditable, sin introducir otro workload.
        ...
        # E-009 mantiene la fuente accesible, pero hace visible un candidato cuyo bundle
        # viola el contrato de revisión. La carga operacional sigue siendo el churn canónico.
        if self.invalid_candidate_at_seconds > 0:
            if normalized_profile != _LATEST_NARROW:
        ...
        # E-010 combina churn operacional con una adopción estructural cuya promoción de cache se interrumpe por pérdida de lease.
        # Estas guardas congelan la geometría acordada para que la prueba no mezcle otras presiones ni produzca grupos parciales.
        if self.lease_loss_adoption_at_seconds > 0:
            if normalized_profile != _LATEST_NARROW:
        ...
        # F-001 y F-002 son contratos de soak temporal aislado con la misma duración
        # autoritativa. Adelantamos sólo esta regla para que una duración no canónica
        # informe el contrato específico antes de calcular la geometría genérica de ventanas.
        ...
        # F-002 conserva el mismo modelo temporal validado por F-001 y cambia sólo
        # el volumen objetivo a 1000 alarmas. No habilita presión adicional: mantiene
        # Shared Latest, una única partición, configuración estable y sin control-plane.
        ...
        # La presión Management histórica continúa usando latest-narrow. F-010 es la única
        # excepción autorizada: combina Management/Decisions con el perfil físico integrado
        # para la qualification final sin abrir esa combinación al resto de escenarios.
        if self.management_action_at_seconds > 0:
            if normalized_profile not in (_LATEST_NARROW, _F010_PHYSICAL_INTEGRATED):
        ...
        # El ID F-010 congela la geometría completa para impedir variantes accidentales del workload final.
    ...
    # Estas propiedades convierten la configuración temporal en expectativas exactas
    # para que F-001/F-002 puedan reutilizar el mismo contrato sin un scheduler paralelo.
    @property
    ...
    @property
    # Indica que el escenario debe ejecutarse con el runner two-owner de E-010 sobre almacenamiento compartido.
    ...
    @property
    # Estas propiedades concentran el contrato aritmético de E-012. El runner y las
    # métricas consumen los mismos valores y evitan duplicar números mágicos.
...
# Simula una caída persistente del manifest sin tocar el filesystem real.
# Antes del umbral delega normalmente; después fuerza el error de Source esperado por el resolver.
...
# Simula una fuente disponible que, desde el instante programado, publica un manifest
# válido hacia PERF-AC-2 pero entrega un documento cuyo campo revision no coincide.
@dataclass(slots=True)
...
# Cuenta fallas contractuales sólo para el candidato inválido, sin alterar el decoder base.
@dataclass(slots=True)
...
# Instrumenta solamente el harness para demostrar que el LKG se promueve una sola vez.
# La implementación productiva del cache permanece intacta.
@dataclass(slots=True)
...
# Este wrapper deja pasar el bootstrap AC1 y corta exactamente el primer
# replace de AC2 antes de tocar el FileRuntimeRevisionCache real.
@dataclass(slots=True)
...
# Error determinístico del harness: simula que la autoridad durable ya cambió,
# pero la promoción física del cache target no puede completarse.
...
def build_baseline_runtime(
    *,
    ...
    # Sólo F-007 puede inyectar el loader físico; los demás perfiles conservan la construcción sintética existente.
    ...
    if scenario.has_rejected_candidate_pressure:
        ...
        schedule_started_monotonic = time.perf_counter()
        schedule_base_at = datetime.now(UTC).replace(microsecond=0)
    # F-010 comparte un único reloj para Management/Decisions y para publicar PERF-AC-2 a +900 s.
    # F-010 combina el flujo causal de Management/Decision con la adopción usando un solo reloj.
...
# Construye PERF-AC-2 cambiando criticality sólo en el 5% inicial, distribuido en cinco grupos completos.
...
# E-011 mantiene su propio constructor de target para no acoplar el escenario
# al helper semántico de pérdida de lease de E-010.
...
def _build_contracts(scenario: BaselineScenario) -> tuple[AlarmEvaluatorContract, ...]:
    # Cada evaluator consume una señal Latest propia y una de las dos series históricas físicas, sin ensanchar el banco.
    # El perfil físico integrado mantiene 1000 señales Latest, pero comparte sólo dos señales DAILY.
    ...
    # Cada contrato pide una sola señal; el planner fusiona las 1.000 señales únicas en una sola vista Latest de ancho fijo.
...
def _evaluator_key(index: int, *, scenario: BaselineScenario) -> str:
    ...
    # El módulo físico es la única autoridad del mapeo round-robin alarma→señal.
    if scenario.data_profile == _F007_PHYSICAL_WARM:
        signal_column = f007_signal_column_for_alarm(index)
...
# Materializa el documento candidato inválido para que la prueba use el mismo backend
# Local FS que el resto de la campaña; sólo el manifest programado decide cuándo es visible.
