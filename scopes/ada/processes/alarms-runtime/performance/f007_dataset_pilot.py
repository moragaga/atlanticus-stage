...
class ControlledDatasetPilot:
    ...
    def _manifest(
        ...
        *,
        ...
        dispatch_files = [
            item for item in inspection['files'] if item['source'] == 'dispatch.std_shift_state'
        ]
...
def _pi_table(
    ...
    *,
    ...
    pa = imports['pa']
    timestamps = [start + timedelta(seconds=index * interval_seconds) for index in range(row_count)]
...
def _file_hash_entries(root: Path) -> tuple[tuple[str, str], ...]:
    return tuple(
        (path.relative_to(root).as_posix(), _sha256_file(path)) for path in _parquet_files(root)
    )
