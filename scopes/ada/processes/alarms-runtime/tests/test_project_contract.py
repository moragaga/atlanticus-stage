...
def test_project_contract_pins_productive_dependencies() -> None:
    ...
    assert not any(
        item.startswith('ada-operational-data-sources==') for item in project['dependencies']
    )
