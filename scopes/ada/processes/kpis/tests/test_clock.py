...
def test_pi_web_api_clock_is_empty_until_source_watermark_exists(tmp_path) -> None:
    ...
    assert snapshot.source_watermarks[KpiSource.PI_INTERPOLATED] is None
    assert snapshot.source_watermarks[KpiSource.PI_RECORDED] is None
...
def test_pi_web_api_clock_uses_source_state_for_both_pi_views(tmp_path) -> None:
    ...
    assert snapshot.source_watermarks[KpiSource.PI_INTERPOLATED] == expected
    assert snapshot.source_watermarks[KpiSource.PI_RECORDED] == expected
...
def test_notpii_clock_is_driven_by_interpolated_stream_not_manifest_max(tmp_path) -> None:
    ...
    assert snapshot.source_watermarks[KpiSource.PI_INTERPOLATED].text == '2026-08-19T20:11:30Z'
    assert snapshot.source_watermarks[KpiSource.PI_RECORDED].text == '2026-08-19T20:12:00Z'
