# Resuelve la configuración local o desplegada del proceso KPI siguiendo el bootstrap estándar de Atlanticus.
# En Azure solo intenta resolver desde Key Vault los secretos que pertenecen al contrato de configuración de este proceso.
...
from ada.processes.kpis.settings import configuration_specs
...
def _required_bootstrap_value(values: Mapping[str, str], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise KpiProcessConfigurationError(f'{key} is required to resolve Key Vault')
    if value != value.strip():
        raise KpiProcessConfigurationError(f'{key} must not contain surrounding whitespace')
    return value
...
def _require_absolute_volume_path(configuration: ResolvedConfiguration) -> ResolvedConfiguration:
    ...
    if not configured_path.is_absolute():
        raise KpiProcessConfigurationError('VOLUMEN_PATH must be an absolute path')
