...
ARTIFACT_PATH="${ARTIFACT_ROOT}/kpis"
...
echo "[1/6] Applying safe Ruff fixes to KPI process source"
...
echo "[2/6] Formatting KPI process source"
uv run --python "${PYTHON_VERSION}" --no-python-downloads --no-project \
    --with "ruff==${RUFF_VERSION}" \
...
uv run --python "${PYTHON_VERSION}" --no-python-downloads --no-sync python -c \
    "import sys; import ada.processes.kpis; assert sys.version_info[:3] == (3, 14, 2), sys.version"
