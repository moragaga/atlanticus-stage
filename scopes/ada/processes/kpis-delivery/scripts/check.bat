...
set "ARTIFACT_PATH=%ARTIFACT_ROOT%\kpis-delivery"
...
uv run --python %PYTHON_VERSION% --no-python-downloads --no-sync python -c "import sys; import ada.processes.kpis_delivery; assert sys.version_info[:3] == (3, 14, 2), sys.version" || exit /b 1
