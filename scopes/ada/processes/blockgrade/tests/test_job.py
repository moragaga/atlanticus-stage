...
class _Planner(BlockgradePlanner):
...
def _source(definition) -> BlockgradeSourcePlan:
...
def _committed(source_key: str) -> BlockgradeSourceExecutionResult:
    target = DatasetTarget(
        dataset=DatasetKey(namespace=('blockgrade',), name=source_key), materialization='latest'
    )
    ...
    return BlockgradeSourceExecutionResult(
        ...
        source_row_count=1,
        publications=(BlockgradePublicationResult(publication=publication),),
    )
...
def test_failed_source_does_not_block_next_source(snapshot_definition) -> None:
    ...
    job.run_iteration(context)
    context.iteration = {}
    with pytest.raises(BlockgradeProcessError, match='1 source failure'):
        job.run_iteration(context)
