...
def test_state_is_persisted_independently_per_source(tmp_path) -> None:
    ...
    assert store.path_for(StateKey(namespace=('producers', 'blockgrade'), name='source_a')).exists()
    assert BlockgradeProducerState(store=store).source_state('source_b').revision == 0


def test_marker_can_advance_without_material_revision(tmp_path) -> None:
    state = BlockgradeProducerState(store=AtomicStateStore(volume_path=tmp_path, application='ada'))
