...
def test_curation_casts_blockgrade_values_and_normalizes_datetime(shift_definition) -> None:
    raw = pa.table(
        {
            'shift_id': [260817002],
    ...
    curated = curate_blockgrade_table(definition=shift_definition, table=raw)
...
def test_required_column_rejects_nulls(shift_definition) -> None:
    ...
    with pytest.raises(BlockgradeSchemaError, match='required column shift_id'):
        curate_blockgrade_table(definition=shift_definition, table=raw)
