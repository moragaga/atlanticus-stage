...
set "ARTIFACT_PATH=%ARTIFACT_ROOT%\blockgrade"
...
uv run --python "%PYTHON_VERSION%" --no-python-downloads --no-sync python -c "import sys; import ada.processes.blockgrade; assert sys.version_info[:3] == (3, 14, 2), sys.version" || exit /b 1
...
echo Blockgrade transport artifact validated: %ARTIFACT_PATH%
exit /b 0
