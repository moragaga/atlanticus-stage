...
_STATE_NAMESPACE = ('producers', 'blockgrade')


@dataclass(frozen=True, slots=True)
class BlockgradeSourceState:
    source_key: str
    revision: int = 0
...
class BlockgradeProducerState:
    def __init__(
        ...
        *,
        ...
        self._cache: dict[str, BlockgradeSourceState] = {}

    def source_state(self, source_key: str) -> BlockgradeSourceState:
        ...
        state = (
            BlockgradeSourceState(source_key=normalized_key)
            if document is None
    ...
    def commit_source(
        ...
        *,
        ...
        next_state = BlockgradeSourceState(
...
def _state_from_value(source_key: str, value: Mapping[str, object]) -> BlockgradeSourceState:
    if value.get('producer') != 'blockgrade':
        raise ValueError('Blockgrade producer state has an invalid producer')
    if value.get('source_key') != source_key:
        raise ValueError('Blockgrade producer state source_key does not match its state path')
    return BlockgradeSourceState(
...
def _state_value(state: BlockgradeSourceState) -> dict[str, object]:
    return {
        'producer': 'blockgrade',
...
def _optional_marker(value: object) -> SqlTableChangeMarker | None:
    ...
    if not isinstance(value, Mapping):
        raise ValueError('Blockgrade producer state source_change_marker must be a mapping or null')
...
def _optional_datetime(value: object) -> datetime | None:
    if value is None:
        return None
    if not isinstance(value, str):
        raise ValueError('Blockgrade producer state timestamp must be a string or null')
    try:
        parsed = datetime.fromisoformat(value.replace('Z', '+00:00'))
    except ValueError as error:
        raise ValueError('Blockgrade producer state timestamp is invalid') from error
    return _normalize_utc(parsed)


def _normalize_utc(value: datetime) -> datetime:
    if not isinstance(value, datetime) or value.tzinfo is None:
        raise ValueError('Blockgrade producer state timestamp must be timezone-aware')
