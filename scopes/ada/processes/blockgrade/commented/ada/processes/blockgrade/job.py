...
from ada.processes.blockgrade.contracts import BlockgradeSourceExecutor
from ada.processes.blockgrade.errors import BlockgradeProcessError
from ada.processes.blockgrade.models import BlockgradeExecutionPlan, BlockgradeSourceDefinition
...
_PLAN_MEMORY_KEY = 'blockgrade.execution_plan'
_CURSOR_MEMORY_KEY = 'blockgrade.execution_cursor'
_FAILURES_MEMORY_KEY = 'blockgrade.execution_failures'
_FIRST_FAILURE_MEMORY_KEY = 'blockgrade.execution_first_failure'
...
class BlockgradeJob:
    def __init__(
        ...
        *,
        ...
        executor: BlockgradeSourceExecutor,
    ) -> None:
        if not definitions or not all(
            isinstance(definition, BlockgradeSourceDefinition) for definition in definitions
        ):
            raise ValueError('definitions must contain at least one BlockgradeSourceDefinition')
        if not isinstance(planner, BlockgradePlanner):
            raise TypeError('planner must be a BlockgradePlanner')
        if not isinstance(producer_state, BlockgradeProducerState):
            raise TypeError('producer_state must be a BlockgradeProducerState')
        if not isinstance(executor, BlockgradeSourceExecutor):
            raise TypeError('executor must implement BlockgradeSourceExecutor')
    ...
    def run_iteration(self, context: JobRuntimeContext) -> None:
        ...
        try:
            ...
            context.logger.exception(
                'Blockgrade source processing failed',
                error,
                event_name='blockgrade.source.failed',
        ...
        if result.source_key != source_plan.definition.source_key:
            raise BlockgradeProcessError('Blockgrade source executor returned an unexpected source_key')
    ...
    @staticmethod
    def _initialize_plan_facts(context: JobRuntimeContext, plan: BlockgradeExecutionPlan) -> None:
