...
from ada.processes.dispatch.models import DispatchLoadStrategy, DispatchSourceDefinition, DispatchSourcePlan
...
class DispatchSqlReader:
    def __init__(
        self,
        *,
        sql: SqlClient,
        retry_policy: DispatchSqlRetryPolicy | None = None,
        ...
        self._sql = sql
        self._retry_policy = retry_policy or DispatchSqlRetryPolicy()
    ...
    def _read_source_once(
        ...
        *,
        ...
        with self._sql.iter_batches(statement, parameters) as stream:
            for batch in stream:
                ...
                if row_count > self._max_rows:
                    raise DispatchSqlReadError(
                        f'Dispatch source exceeded the configured row limit: {plan.definition.source_key}'
                    )
                tables.append(table)
        if context is not None:
            context.raise_if_cancelled()
        ...
        try:
            ...
            raise DispatchSqlReadError('SQL batches could not be combined as Arrow data') from error
...
def _build_select(plan: DispatchSourcePlan) -> tuple[str, tuple[object, ...]]:
    ...
    if definition.load_strategy is DispatchLoadStrategy.SHIFT_WINDOW:
        if definition.shift_id_column is None:
            raise DispatchSqlReadError('shift_window source has no shift_id_column')
...
def _batch_to_table(batch: SqlBatch, *, expected_columns: tuple[str, ...]) -> pa.Table:
    if batch.columns != expected_columns:
        raise DispatchSqlReadError('SQL source result columns do not match the Dispatch catalog')
    ...
    try:
        ...
        raise DispatchSqlReadError('SQL batch could not be converted to Arrow') from error


def _quote_identifier_path(value: str) -> str:
    parts = tuple(part.strip() for part in value.split('.'))
    if len(parts) != 2 or any(not part for part in parts):
        raise DispatchSqlReadError('SQL source table identifier must use schema.table format')
...
def _quote_identifier(value: str) -> str:
    ...
    if not normalized:
        raise DispatchSqlReadError('SQL identifier is invalid')
