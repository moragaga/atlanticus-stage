# Watermark privado del Historian bajo .runtime/state/kpis-historian.
...
class KpiHistorianCommitStore:
    ...
    def read_watermark(self) -> KpiWatermark | None:
        ...
        if set(document.value) != {'watermark_utc'}:
            raise KpiHistorianWatermarkError(
                'KPI historian committed watermark state has unexpected or missing fields'
            )
        try:
    ...
    def commit_watermark(self, watermark: KpiWatermark) -> KpiWatermark:
        ...
        if current is not None and watermark < current:
            raise KpiHistorianWatermarkError(
                'KPI historian committed watermark must not move backwards'
            )
        if current == watermark:
            return current
        self._store.replace(_HISTORIAN_WATERMARK_KEY, watermark.as_document())
        return watermark
