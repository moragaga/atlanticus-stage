...
set ARTIFACT_ROOT=%REPOSITORY_ROOT%rtifacts\processes
set ARTIFACT_PATH=%ARTIFACT_ROOT%\kpis-historian
set BUNDLER=%REPOSITORY_ROOT%\scopesda\scripts\processes\process_bundle.py
...
uv run --python %PYTHON_VERSION% --no-python-downloads --no-sync python -c "import sys; import ada.processes.kpis_historian; assert sys.version_info[:3] == (3, 14, 2), sys.version" || exit /b 1
popd
