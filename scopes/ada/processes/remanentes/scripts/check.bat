...
set "ARTIFACT_PATH=%ARTIFACT_ROOT%\remanentes"
...
uv run --python "%PYTHON_VERSION%" --no-python-downloads --no-sync python -c "import sys; import ada.processes.remanentes; assert sys.version_info[:3] == (3, 14, 2), sys.version" || exit /b 1

if exist .venv rmdir /s /q .venv
for /d /r src %%D in (*.egg-info) do if exist "%%D" rmdir /s /q "%%D"
for /d /r src %%D in (*.dist-info) do if exist "%%D" rmdir /s /q "%%D"
for /d /r src %%D in (__pycache__) do if exist "%%D" rmdir /s /q "%%D"
for /r src %%F in (*.pyc) do if exist "%%F" del /q "%%F"
for /d /r src %%D in (*.egg-info) do (
    echo Transport bundle contains generated package metadata after validation: %ARTIFACT_PATH% 1>&2
    exit /b 1
)

echo Remanentes transport artifact validated: %ARTIFACT_PATH%
exit /b 0
