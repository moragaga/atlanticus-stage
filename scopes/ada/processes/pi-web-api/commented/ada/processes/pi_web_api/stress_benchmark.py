# Espejo pedagógico del benchmark temporal de presión PI Web API.
# Los tags de esta lista son señales interpoladas reales entregadas para la prueba; se conserva NUMBER/TEXT.
# PI se consulta únicamente por esos tags físicos; el fan-out a 1000 columnas sucede después de adquirir.
# El planner recorre el intervalo histórico desde el comienzo hacia adelante, una ventana máxima por iteración.
# La materialización usa MONTHLY como corteza para evitar el corte diario y acumular un día completo en un solo artefacto.
# El job de stress se detiene solo después de confirmar el watermark final.
# Todo el benchmark exige una APPLICATION aislada para no contaminar state ni datasets productivos.
# Con el flag deshabilitado, los demás parámetros de stress ni siquiera se interpretan.
# Las métricas stress_* separan adquisición, fan-out, materialización y tamaño final del artefacto.
...
class PiStressBenchmarkSettings:
    ...
    def from_configuration(
        ...
        kind = (
            configuration.get('PI_WEB_API_STRESS_KIND', 'capacity') or 'capacity'
        ).strip().lower()
        if kind not in {'capacity', 'io'}:
            raise PiWebApiProcessConfigurationError(
                'PI_WEB_API_STRESS_KIND must be capacity or io'
            )
        try:
        ...
        if end_utc is None:
            raise PiWebApiProcessConfigurationError(
                'PI_WEB_API_STRESS_END_UTC is required when '
                'PI_WEB_API_STRESS_BENCHMARK is enabled'
            )
        return cls(
            enabled=enabled,
            kind=kind,
...
class PiStressBenchmarkPlanner(PiSlotPlanner):
    ...
    def __post_init__(self) -> None:
        ...
        start_utc = end_utc - timedelta(hours=self.lookback_hours) + timedelta(
            seconds=self.interpolation_seconds
        )
    ...
    def benchmark_start_utc(self) -> datetime:
        return self.benchmark_end_utc - timedelta(hours=self.lookback_hours) + timedelta(
            seconds=self.interpolation_seconds
        )
    ...
    def plan(
        ...
        *,
        ...
        last_slot = first_pending + timedelta(
            seconds=(slot_count - 1) * self.interpolation_seconds
        )
...
def _build_logical_catalog(
    *,
    ...
    physical = tuple(
        ...
        and definition.extraction_mode is PiExtractionMode.INTERPOLATED
    )
    if not physical:
        raise PiWebApiProcessConfigurationError(
