# Espejo pedagógico: proyecta ambos modos al eje común y publica Parquet de forma idempotente.
...
# Parquet no conserva timestamp[s] como el mismo tipo Arrow en el round-trip. Usamos microsegundos como tipo físico compatible; los valores siguen alineados a los slots de segundos definidos por PI.
...
class PiWebApiMaterializer:
    ...
    # La publicación proyecta primero los datos a un eje común y luego confirma cada target.
    # Si el proceso muere a mitad, el watermark no avanza y el replay vuelve a pasar por merges idempotentes.
    def publish(
        self,
        *,
    ...
    # latest reemplaza un snapshot; daily/monthly integran por timestamp para que repetir
    # exactamente la misma ventana no agregue filas duplicadas.
    def _publish_mode(
        self,
        *,
    ...
    # Solo se inspecciona metadata Parquet. Una partición existente conserva su schema histórico;
    # una nueva parte desde el schema del catálogo vigente.
...
# Recorded conserva primero timestamps nativos distintos. Al caer varios eventos en el mismo
# slot final, se selecciona el timestamp nativo más reciente y se deja traza del conflicto.
...
# Un alias retirado permanece en la partición ya abierta con nulos nuevos. Un alias nuevo se agrega.
# Cambiar el tipo de un alias existente es drift incompatible y falla antes de escribir.
...
# El schema resuelto gobierna el orden y tipos físicos. Columnas históricas sin definición activa
# se rellenan con null sin reintroducir lógica de negocio en el store Parquet.
def _build_table(
    *,
    slots: tuple[datetime, ...],
    definitions: tuple[PiTagDefinition, ...],
