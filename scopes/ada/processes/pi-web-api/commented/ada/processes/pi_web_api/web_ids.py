...
# Cache persistente y reconstruible de tag_name -> WebID. No es estado operacional ni fuente de verdad.
...
from ada.processes.pi_web_api.errors import PiWebApiWebIdRegistryError
from atlanticus.runtime import RuntimeConfiguration

_SCHEMA_VERSION = 1


# Mantiene el archivo en memoria durante la vida del proceso y solo reescribe cuando cambia.
class WebIdRegistry:
    def __init__(self, *, path: str | Path) -> None:
        self._path = Path(path)
        if not self._path.is_absolute():
            raise PiWebApiWebIdRegistryError('WebID registry path must be absolute')
        self._lock = Lock()
        self._entries: dict[str, str] | None = None

    @classmethod
    def from_runtime_configuration(cls, configuration: RuntimeConfiguration) -> WebIdRegistry:
        if not isinstance(configuration, RuntimeConfiguration):
            raise TypeError('configuration must be a RuntimeConfiguration')
        return cls(
            path=configuration.runtime_root / 'cache' / 'pi-web-api' / 'webids.json'
        )
    ...
    def lookup(self, tag_names: tuple[str, ...]) -> Mapping[str, str]:
        if not isinstance(tag_names, tuple) or any(
            ...
            raise PiWebApiWebIdRegistryError(
    ...
    # Merge incremental: nunca poda entradas que el catálogo actual no use.
    ...
    def _read(self) -> dict[str, str]:
        ...
        try:
            ...
            raise PiWebApiWebIdRegistryError('WebID registry could not be read') from error
        if not isinstance(payload, Mapping):
            raise PiWebApiWebIdRegistryError('WebID registry must be a JSON object')
        if set(payload) != {'schema_version', 'web_ids'}:
            raise PiWebApiWebIdRegistryError('WebID registry has unexpected or missing fields')
        if payload.get('schema_version') != _SCHEMA_VERSION:
            raise PiWebApiWebIdRegistryError('WebID registry schema version is not supported')
        web_ids = payload.get('web_ids')
        if not isinstance(web_ids, Mapping):
            raise PiWebApiWebIdRegistryError('WebID registry web_ids must be a JSON object')
        return _normalize_entries(web_ids)

    # Escritura atómica mediante temporal + fsync + os.replace.
    def _replace(self, entries: Mapping[str, str]) -> None:
        ...
        try:
            ...
            raise PiWebApiWebIdRegistryError('WebID registry could not be written') from error
        finally:
...
def _normalize_entries(entries: Mapping[str, object]) -> dict[str, str]:
    if not isinstance(entries, Mapping):
        raise PiWebApiWebIdRegistryError('WebID entries must be a mapping')
    normalized: dict[str, str] = {}
    seen: set[str] = set()
    for raw_tag_name, raw_web_id in entries.items():
        if not isinstance(raw_tag_name, str) or not raw_tag_name:
            raise PiWebApiWebIdRegistryError('WebID registry tag names must be non-empty text')
        ...
        if not isinstance(raw_web_id, str) or not raw_web_id:
            raise PiWebApiWebIdRegistryError('WebID registry values must be non-empty text')
        ...
        if key in seen:
            raise PiWebApiWebIdRegistryError('WebID registry contains duplicate tag names')
        seen.add(key)
