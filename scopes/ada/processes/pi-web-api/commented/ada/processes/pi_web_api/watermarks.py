...
# Estado durable: cursor privado del productor y watermark público de la fuente PI.
...
# Cursor privado: indica desde qué slot debe continuar exclusivamente este job.
...
# Watermark público: otros procesos pueden leer hasta qué slot PI ya está disponible.
class PiSourceState:
    def __init__(self, *, store: AtomicStateStore) -> None:
        if not isinstance(store, AtomicStateStore):
            raise TypeError('store must be an AtomicStateStore')
...
# Publica primero la fuente y luego confirma el cursor privado para preferir repetición sobre huecos.
