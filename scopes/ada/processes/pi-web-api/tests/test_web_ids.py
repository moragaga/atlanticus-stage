...
from ada.processes.pi_web_api import PiWebApiWebIdRegistryError, WebIdRegistry
...
def test_registry_rejects_corrupt_documents_instead_of_silently_rebuilding(tmp_path: Path) -> None:
    ...
    with pytest.raises(PiWebApiWebIdRegistryError, match='could not be read'):
        WebIdRegistry(path=path).current()


def test_registry_requires_an_absolute_path() -> None:
    with pytest.raises(PiWebApiWebIdRegistryError, match='must be absolute'):
