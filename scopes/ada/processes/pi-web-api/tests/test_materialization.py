...
def test_materializer_projects_recorded_to_common_slots_and_is_idempotent(tmp_path) -> None:
    catalog = PiCatalog(
    ...
    acquisition = PiAcquisitionResult(
        interpolated=(
            PiSample('TAG_A', datetime(2026, 8, 15, 10, 0, 0, tzinfo=UTC), 1),
            PiSample('TAG_A', datetime(2026, 8, 15, 10, 0, 10, tzinfo=UTC), 2),
        ),
        recorded=(
            PiSample('TAG_B', datetime(2026, 8, 15, 10, 0, 1, tzinfo=UTC), 10),
            PiSample('TAG_B', datetime(2026, 8, 15, 10, 0, 8, tzinfo=UTC), 11),
            PiSample('TAG_B', datetime(2026, 8, 15, 10, 0, 12, tzinfo=UTC), 12),
        ),
    )
    ...
    assert first.recorded_bucket_conflict_count == 1
    assert all(result.status is PublicationStatus.UNCHANGED for result in second.publications)

    interpolated_dataset = materializer.dataset_for(PiExtractionMode.INTERPOLATED)
    assert interpolated_dataset is not None
    interpolated_daily = interpolated_dataset.resolve_target(
        materialization='daily',
        partition={'year': '2026', 'month': '08', 'day': '15'},
    )
    interpolated_table = runtime.read_table(
        definition=interpolated_dataset,
        target=interpolated_daily,
    ).table
    assert interpolated_table.schema.field('timestamp_utc').type == pa.timestamp('us', tz='UTC')
    assert interpolated_table.to_pydict() == {
        'timestamp_utc': [
            datetime(2026, 8, 15, 10, 0, 0, tzinfo=UTC),
            datetime(2026, 8, 15, 10, 0, 10, tzinfo=UTC),
        ],
    ...
    recorded_daily = recorded_dataset.resolve_target(
        materialization='daily',
        partition={'year': '2026', 'month': '08', 'day': '15'},
    )
    recorded_table = runtime.read_table(
        definition=recorded_dataset,
        target=recorded_daily,
    ).table
    assert recorded_table.to_pydict() == {
        'timestamp_utc': [
            datetime(2026, 8, 15, 10, 0, 0, tzinfo=UTC),
            datetime(2026, 8, 15, 10, 0, 10, tzinfo=UTC),
        ],
