...
class PiStressIoBenchmarkJob(PiWebApiJob):
    ...
    def __post_init__(self) -> None:
        ...
        if self.benchmark_end_utc.tzinfo is None or self.benchmark_end_utc.utcoffset() != timedelta(
            0
        ):
        ...
        if self.benchmark_end_utc.microsecond:
            raise PiWebApiProcessConfigurationError(
                'benchmark_end_utc must not contain microseconds'
            )
        if not isinstance(self.interpolation_seconds, int) or isinstance(
            self.interpolation_seconds, bool
        ):
            raise TypeError('interpolation_seconds must be an integer')
        if self.interpolation_seconds <= 0:
            raise PiWebApiProcessConfigurationError(
                'interpolation_seconds must be greater than zero'
            )
...
def run_io_case(
    *,
    ...
    chunks = tuple(
        tags[offset : offset + chunk_limit] for offset in range(0, len(tags), chunk_limit)
    )
    ...
    if worker_count == 1:
        ...
        with ThreadPoolExecutor(
            max_workers=worker_count, thread_name_prefix='pi-stress-io'
        ) as executor:
            futures = {
                executor.submit(
                    _fetch_chunk,
    ...
    *,
    ...
    try:
        records = client.streamsets.get_interpolated(
            ...
            end_time_utc=window.last_slot_utc + timedelta(seconds=window.interpolation_seconds),
...
def _build_io_windows(
    *,
    ...
    hour_window = PiAcquisitionWindow(
        first_slot_utc=end_utc - timedelta(seconds=(_IO_HOUR_SLOTS - 1) * interpolation_seconds),
        last_slot_utc=end_utc,
        interpolation_seconds=interpolation_seconds,
        recovery_truncated=False,
    )
