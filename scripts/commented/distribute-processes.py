...
# Error de frontera usado para detener una entrega antes de publicar un paquete incompleto.
class DistributionError(RuntimeError):
    pass


# Contrato estable que relaciona cada proceso con su número de job de plataforma.
...
# Catálogo único y estable: los números no dependen del subconjunto seleccionado.
PROCESS_JOBS = (
...
# Resuelve la selección solicitada conservando el orden y numeración del catálogo.
def select_jobs(processes: tuple[str, ...], *, include_all: bool) -> tuple[ProcessJob, ...]:
...
# Todas las entregas se construyen dentro de distribution/<nombre> en Atlanticus.
...
# Valida todo lo necesario antes de tocar una distribución existente.
def validate_distribution(
    *,
    ...
    commented_local_script = (
        repository_root / 'deployment' / 'distribution' / 'commented' / 'local-process.sh'
    )
    ...
    for job in jobs:
        ...
        if not (artifact_root / 'wheels').is_dir():
            raise DistributionError(f'process artifact wheels must be a directory: {artifact_root}')
        if not (artifact_root / 'src').is_dir():
            raise DistributionError(f'process artifact src must be a directory: {artifact_root}')
        _container_resources(artifact_root / 'pyproject.toml')


# Construye en staging y publica atómicamente el paquete autocontenido completo.
def distribute(
    *,
    repository_root: Path,
    name: str,
...
# Excluye residuos locales, secretos y configuración propiedad del consumidor.
...
# Conserva únicamente los archivos deliberadamente editables del consumidor al redistribuir.
...
def _validate_staged_process(process_root: Path, job: ProcessJob) -> None:
    for relative in REQUIRED_ARTIFACT_ENTRIES:
        if not (process_root / relative).exists():
            raise DistributionError(f'staged process is incomplete ({relative}): {process_root}')
    forbidden = tuple(
        path
...
# Traduce el catálogo Atlanticus al contrato legacy exacto de services.json.
...
def _validate_services(path: Path, jobs: tuple[ProcessJob, ...]) -> None:
    try:
        ...
        raise DistributionError(f'could not validate generated services file: {path}') from error
    expected = [_service(job) for job in jobs]
    if payload != expected:
        raise DistributionError(f'generated services file does not match selected jobs: {path}')


# Inyecta el Compose E2E y su runner en la misma entrega, sin un segundo paso de generación.
def _write_local_deployment(
    *,
    ...
    definitions = tuple(
        (
            job,
            _container_resources(staging_root / 'processes' / job.name / 'pyproject.toml'),
        )
        for job in jobs
    )
...
# Lee los recursos operacionales declarados por el artifact; usa los defaults centrales si no hay override.
def _container_resources(pyproject_path: Path) -> ContainerResources:
    ...
    if system_profile not in ALLOWED_SYSTEM_PROFILES:
        raise DistributionError(f'container system profile is invalid: {pyproject_path}')
    resources = container.get('resources', {})
    if not isinstance(resources, dict):
        raise DistributionError(f'container resources must be a table: {pyproject_path}')
    ...
    if isinstance(cpus, bool) or not isinstance(cpus, (int, float)) or cpus <= 0:
        raise DistributionError(f'container cpus must be greater than zero: {pyproject_path}')
...
# Genera un Compose autocontenido que ejecuta todos los procesos seleccionados una vez.
def _render_compose(
    distribution_name: str,
    ...
    *,
    ...
    services = '\n'.join(
        _render_local_service(distribution_name, job, resources, volume_mode=volume_mode)
        for job, resources in definitions
    )
    suffix = f'\nvolumes:\n  {RUNTIME_VOLUME_KEY}:\n' if volume_mode == 'named' else ''
    return f'name: atlanticus-{distribution_name}-local\nservices:\n{services}\n{suffix}'
...
def _validate_local_deployment(staging_root: Path, jobs: tuple[ProcessJob, ...]) -> None:
    local_root = staging_root / 'local-deployment'
    for compose_name in ('compose.yaml', 'compose.bind.yaml'):
        ...
        for job in jobs:
            if f'  {job.name}:' not in compose:
                raise DistributionError(f'generated local Compose is missing service: {job.name}')
            if f'FILENAME: {job.name}' not in compose:
                raise DistributionError(f'generated local Compose has invalid selector: {job.name}')
            if f'../processes/{job.name}/.env' not in compose:
                raise DistributionError(f'generated local Compose has invalid env path: {job.name}')
...
# Reemplazo con backup temporal para evitar dejar una distribución parcialmente publicada.
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--repository-root', type=Path, default=_repository_root_from_script())
    parser.add_argument('--all', action='store_true', dest='include_all')
    return parser.parse_args()


# Entrada CLI utilizada por el wrapper humano scripts/distribute-processes.sh.
def main() -> None:
