...
def test_environment_file_is_required_in_artifact_by_convention(tmp_path: Path) -> None:
    ...
    with pytest.raises(LocalDeploymentError, match=r'artifacts/processes/dispatch/\.env'):
...
def test_commented_generator_is_structurally_equivalent() -> None:
    ...
    production = ast.dump(
        ast.parse((deployment_root / 'generate_compose.py').read_text(encoding='utf-8')),
        include_attributes=False,
    )
    commented = ast.dump(
        ast.parse(
            (deployment_root / 'commented' / 'generate_compose.py').read_text(encoding='utf-8')
        ),
        include_attributes=False,
    )
...
def test_dockerignore_exposes_only_dockerfile_and_processes() -> None:
    ...
    assert dockerignore.splitlines() == ['*', '!Dockerfile', '!processes/', '!processes/**']
