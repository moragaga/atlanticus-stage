...
# El sufijo crea conexiones nombradas sin permitir caracteres que alteren claves.
...
class HttpSettings:
    ...
    def __post_init__(self) -> None:
        # La construcción directa exige tipos finales; solo from_mapping interpreta texto.
        ...
        for name in (
            ...
            # Los timeouts finitos evitan presupuestos ambiguos o infinitos.
    ...
    def from_mapping(
        ...
        *,
        ...
        # Todas las claves se resuelven antes de leer secretos o valores opcionales.
...
def _build_http_configuration_keys(*, suffix: str | None = None) -> HttpConfigurationKeys:
    # Este helper permanece interno porque from_mapping es el único consumidor público.
    normalized_suffix = _normalize_configuration_suffix(suffix)
...
def _normalize_base_url(value: str, *, allow_insecure_http: bool) -> str:
    ...
    # La barra final garantiza que HTTPX resuelva endpoints relativos bajo el path base.
...
def _parse_mapping_number(
    ...
    *,
    ...
    # Un mapping puede provenir del entorno y por eso acepta una representación textual.
...
def _require_positive_number(value: Any, field_name: str) -> float:
    # bool hereda de int en Python; se rechaza explícitamente para no aceptar True como 1.
...
def _validate_authentication(
    *,
    ...
    # Cada modo admite una única combinación para evitar credenciales sobrantes o ambiguas.
