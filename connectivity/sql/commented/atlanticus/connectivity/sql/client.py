...
# El warning conocido de mssql-python se filtra sólo dentro de su import lazy.
import warnings
...
class SqlBatchStream(Iterator[SqlBatch]):
    ...
    # Si el cuerpo del with ya falló, un error secundario de cierre no debe ocultarlo.
    def __exit__(
        self,
...
class SqlClient:
    ...
    # Cada operación abre su propia conexión; mssql-python mantiene su pooling interno.
    def _connect(self) -> tuple[Any, type[BaseException]]:
        driver = _load_mssql_driver()
        connection: Any | None = None
        try:
            # Driver y Connection Timeout se traducen sólo para el runtime de mssql-python.
            ...
            connection = driver.connect(
                connection_string,
                autocommit=True,
                # mssql-python recibe el timeout de conexión como argumento, no como keyword ODBC.
                timeout=0 if connection_timeout_seconds is None else connection_timeout_seconds,
                # Conserva compatibilidad con PyODBC: UNIQUEIDENTIFIER sale como str.
                native_uuid=False,
            )
            # El timeout de consulta sigue siendo un contrato separado de SqlSettings.
...
# mssql-python expone diagnóstico también en driver_error/ddbc_error, no sólo args.
...
# El driver se carga tarde para que importar settings/modelos no dependa del runtime nativo.
