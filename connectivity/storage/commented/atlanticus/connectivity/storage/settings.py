...
# La connection string se considera un secreto opaco: se valida lo mínimo necesario y se
# preserva exactamente, sin strip ni reconstrucciones.
...
# SAS necesita además la URL pública de la cuenta. La URL sí puede normalizarse porque no contiene
# el token; el token se conserva exactamente y se excluye del repr.
@dataclass(frozen=True, slots=True)
...
# StorageSettings recibe valores finales. No conoce variables de entorno, prefixes, Key Vault ni
# nombres como source/output/archive; todo eso pertenece a composición.
@dataclass(frozen=True, slots=True)
